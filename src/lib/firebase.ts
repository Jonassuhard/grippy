...
export async function getProfile(uid: string): Promise<UserProfile | null> {
  ...
  return { lastname: data.lastname || "", name: data.name || "", age: data.age || "", dischargeDate: data.dischargeDate || "" };
}

// ─── Sessions ───────────────────────────────────────────
export async function saveSession(session: Omit<Session, "startedAt"> & { startedAt?: Date }) {
  if (!db) return;
  await addDoc(collection(db, "sessions"), { ...session, startedAt: serverTimestamp(), completedAt: serverTimestamp() });
}
...
export async function getAllPatients(): Promise<PatientData[]> {
  ...
  return snapshot.docs.map((d) => {
    ...
    return {
      uid: d.id,
      profile: { lastname: data.lastname || "", name: data.name || "", age: data.age || "", dischargeDate: data.dischargeDate || "" },
...
export async function getPatientSessions(uid: string): Promise<Session[]> {
  if (!db) return [];
  const q = query(collection(db, "sessions"), where("userId", "==", uid), orderBy("startedAt", "desc"));
  const snapshot = await getDocs(q);
  return snapshot.docs.map((d) => {
    const data = d.data();
    return {
      userId: data.userId, gripType: data.gripType, level: data.level, durationMinutes: data.durationMinutes,
      exercises: data.exercises || [], startedAt: data.startedAt?.toDate() || new Date(), completedAt: data.completedAt?.toDate(), score: data.score,
    };
  });
}
